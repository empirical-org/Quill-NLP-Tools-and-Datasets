...
EXCLUDE_LABELS = set(["Plagiarism",
                      "Irrelevant facts from article",
                      "Insufficient or unintelligible response",
                      "Meat industry is actually positive"])
PROMPT = "Large amounts of meat consumption are harming the environment, because "

@click.command()
@click.argument('input_file')
@click.option('--path', default=None, help='Path to existing train, dev and test files')
def prepare_data(input_file, path):
    ...
    Args:
        ...
        path: the path to existing data sets (with train, dev and test files)
    """
